...
export const handleUpdateRuleById = async (ruleId: string, body: Partial<RuleDto>): Promise<boolean> => {
  let tagsJson = body.tags;
  if (Array.isArray(body.tags)) {
    tagsJson = Object.fromEntries(body.tags.map(tag => [tag, tag]));
  }

  const [affectedRows] = await RuleModel.update({ ...body, tags: tagsJson }, { where: { id: ruleId } });
